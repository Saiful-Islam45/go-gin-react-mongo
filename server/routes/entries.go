...
import (
	...
	"github.com/Saiful-Islam45/go-react-mongo-fullstack/models"
...
func AddEntry(c *gin.Context) {
	...
	if validateErr := validate.Struct(entry); validateErr != nil {
	...
	if insertErr != nil {
		msg := fmt.Sprintf("Insert order entry failed")
...
func UpdateIngredient(c *gin.Context) {
	...
	result, err := entryCollection.UpdateOne(ctx,
		bson.M{"_id": docID},
		bson.D{{
			"$set", bson.D{{"ingredients", ingredient.Ingredients}},
		}},
	)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
